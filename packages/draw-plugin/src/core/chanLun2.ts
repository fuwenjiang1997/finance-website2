...
export function processKlines(klines: KLine[]) {
  ...
  const processed: KLine[] = []

  let lastK: KLine = { ...klines[0] }

  for (let i = 1; i < klines.length; i++) {
    let _current: ProcessedKLine = { ...klines[i] }
    if (lastK.high >= _current.high && lastK.low <= _current.low) {
      lastK.endIndex = i
    } else if (lastK.high <= _current.high && lastK.low <= _current.low) {
      Object.assign(lastK, {
        high: _current.high,
        low: _current.low,
        index: _current.index,
        endIndex: _current.index,
      } as Partial<KLine>)
    } else {
      processed.push(lastK) // 推入栈中
      lastK = _current
    }
  }
  if (lastK) {
    processed.push(lastK)
  }

  return processed
}

export function findRawFenxings(processedKlines: KLine[]): Fenxing[] {
...
export function filterFenxings(rawFenxings: Fenxing[]): Fenxing[] {
  ...
  for (const currentFenxing of rawFenxings) {
    ...
    if (currentFenxing.type === lastFenxing.type) {
      ...
      if (Math.abs(currentFenxing.k.index - lastFenxing.k.index) > 2) {
...
export function findBi(fenxings: Fenxing[], processedKlines: ProcessedKLine[]): Bi[] {
  ...
  for (let i = 1; i < fenxings.length; i++) {
    ...
    console.log('lastBi:', lastBi)
  }

  return biList
}

export function calculateChanLun(klines: KLine[]) {
